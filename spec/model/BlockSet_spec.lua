...
describe("npge.model.BlockSet", function()
    ...
    it("gets block(s) by fragment", function()
        ...
        assert.same(blockset:blockByFragment(f3), nil)
        assert.same(blockset:blocksByFragment(f3), {})
    end)

    it("gets block(s) by fragment (equal fragments)",
    function()
        local s = model.Sequence("s", "ATAT")
        local f1 = model.Fragment(s, 0, 0, 1)
